...
class WeComAdapter(BasePlatformAdapter):
    ...
    def _iter_markdown_source_chunks(self, content: str) -> List[tuple[str, int]]:
        """Return bounded markdown chunks plus consumed source-prefix lengths.

        Native stream overflow fallback needs a source prefix length for partial
        success recovery. The regular chunker adds indicators and can reopen
        code fences, so its displayed chunks are not a direct source-prefix map.
        """
        content_text = "" if content is None else str(content)
        if not content_text:
            return []
        ...
        while pos < total:
            remaining = total - pos
            if remaining <= self.MAX_MESSAGE_LENGTH:
                chunks.append((content_text[pos:], total))
                break
            end = pos + self.MAX_MESSAGE_LENGTH
            split_at = content_text.rfind("\n", pos, end)
            if split_at < pos + self.MAX_MESSAGE_LENGTH // 2:
                split_at = content_text.rfind(" ", pos, end)
            if split_at <= pos:
                split_at = end
            chunk = content_text[pos:split_at]
            next_pos = split_at
            while next_pos < total and content_text[next_pos] in " \t\r\n":
                next_pos += 1
            chunks.append((chunk, next_pos))
            pos = next_pos
        return chunks

    async def _send_reply_markdown_with_confirmed_len(
        self,
        reply_req_id: str,
        content: str,
        ...
        response: Dict[str, Any] = {}
        delivered_len = 0
        for chunk, consumed_len in self._iter_markdown_source_chunks(content):
            response = await self._send_reply_request(
                reply_req_id,
                {
                    "msgtype": "markdown",
            ...
            self._raise_for_wecom_error(response, "send reply markdown")
            delivered_len = consumed_len
        return response, delivered_len
    ...
    async def send_stream_chunk(
        ...
        if overflow_content:
            delivered_overflow_len = 0
            overflow_response: Dict[str, Any] = {}
            try:
                for overflow_chunk, consumed_len in self._iter_markdown_source_chunks(overflow_content):
                    overflow_response = await self._send_reply_request(
                        reply_req_id,
                        {
                            "msgtype": "markdown",
                            "markdown": {"content": overflow_chunk},
                        },
                    )
                    self._raise_for_wecom_error(overflow_response, "send reply markdown")
                    delivered_overflow_len = consumed_len
            except asyncio.TimeoutError:
                self._clear_stream_state(normalized_stream_key)
                return SendResult(
                    success=False,
                    ...
                    raw_response={
                        "confirmed_prefix_len": len(native_content) + delivered_overflow_len,
            ...
            return SendResult(
                ...
                raw_response={
                    ...
                    "confirmed_prefix_len": len(content_text),
                },
            )
