...
class GatewayStreamConsumer:
            ...
            while True:
                ...
                if got_done:
                    ...
                    if _is_intentional_silence_response(
                        ...
                        await self._finalize_native_reply_stream_if_started()
    ...
    async def _send_native_reply_chunk(self, text: str, *, finalize: bool = False) -> bool:
        ...
        if text == self._WECOM_THINKING_PLACEHOLDER:
            if getattr(result, "success", False):
                self._native_stream_started = True
                return True
            self._use_native_reply_streaming = False
            self._fallback_final_send = bool(self._native_prefix)
        ...
        if delivered:
            self._native_stream_started = not finalize
            self._already_sent = True
            self._last_sent_text = text
            return True
        if visible:
    ...
    async def _finalize_native_reply_stream_if_started(self) -> bool:
        """Best-effort finish frame for placeholder-only or suppressed native streams."""
        if (
            not self._use_native_reply_streaming
            or not self._native_stream_started
            or self._native_ambiguous_failure
        ):
            return False
        try:
            return bool(await self._send_native_reply_chunk("", finalize=True))
        except Exception:
            logger.debug("Failed to finalize native reply stream during cleanup", exc_info=True)
            self._use_native_reply_streaming = False
            return False
