...
@pytest.mark.asyncio
async def test_native_final_failure_after_visible_prefix_falls_back_tail_only():
    adapter = _WeComNativeAdapter(
        native_results=[
            SimpleNamespace(success=True),
            SimpleNamespace(success=False, error="finish failed"),
        ]
    )
    consumer = GatewayStreamConsumer(
        adapter,
...
@pytest.mark.asyncio
async def test_native_thinking_frame_finalizes_when_turn_ends_without_content():
    adapter = _WeComNativeAdapter()
    consumer = GatewayStreamConsumer(
        adapter,
        "chat_123",
    ...
    assert [call["content"] for call in adapter.native_calls] == ["THINKING_MESSAGE", ""]
    assert [call["finalize"] for call in adapter.native_calls] == [False, True]
    assert adapter.native_calls[0]["stream_key"] == adapter.native_calls[1]["stream_key"]
    adapter.send.assert_not_awaited()
    adapter.edit_message.assert_not_called()
...
@pytest.mark.asyncio
async def test_native_thinking_frame_finalizes_before_intentional_silence_suppression():
    adapter = _WeComNativeAdapter()
    consumer = GatewayStreamConsumer(
        adapter,
        "chat_123",
